...
import com.meadowsapps.jgameboy.gbc.core.element.cpu.GbcOpcodeTable
import com.meadowsapps.jgameboy.groovy.core.element.cpu.Cpu
import com.meadowsapps.jgameboy.groovy.core.util.*
 ...
 */
@InheritConstructors
class GbcCpu extends GbcCoreElement implements Cpu {
...
//        Opcode op = table.lookup(opcode)
        UInt8 operand1 = mmu().read(PC.word + 1)
        UInt8 operand2 = mmu().read(PC.word + 2)
        boolean jumped = false

        UInt8 d8 = operand1
        UInt16 d16 = (operand2.intValue() << 8) + operand1.intValue()
        UInt8 a8 = operand1
        UInt16 a16 = (operand2.intValue() << 8) + operand1.intValue()
        byte r8 = operand1.byteValue()
        ...
        // LD BC,d16
            case 0x01:
                BC.word = new UInt16(d16)
                break

        // LD (BC),A
            case 0x02:
                mmu().writeByte(AF.hi, BC.word)
                break
        ...
        // LD (HL),B
            case 0x70:
                UInt16 addr = combine(H.value, L.value)
                mmu()[addr] = B.value
                break
...
//        }
    ...
    void handleInterrupts() {
        if (isInterruptTriggered()) {
            ...
            if (ime) {
                ...
                if (isInterruptEnabled(V_BLANK_INTERRUPT)) {
                    disableInterrupt(V_BLANK_INTERRUPT)
                    PC.write(INT40)
                } else if (isInterruptEnabled(LCDC_STATUS_INTERRUPT)) {
                    disableInterrupt(LCDC_STATUS_INTERRUPT)
                    PC.write(INT48)
                } else if (isInterruptEnabled(TIMER_INTERRUPT)) {
                    disableInterrupt(TIMER_INTERRUPT)
                    PC.write(INT50)
                } else if (isInterruptEnabled(SERIAL_INTERRUPT)) {
                    disableInterrupt(SERIAL_INTERRUPT)
                    PC.write(INT58)
                } else if (isInterruptEnabled(JOYPAD_INTERRUPT)) {
                    disableInterrupt(JOYPAD_INTERRUPT)
                    PC.write(INT60)
                }
            }
        }
    ...
    private boolean isInterruptEnabled(int interrupt) {
        int ie = mmu().read(IE)
        return (ie & interrupt) == interrupt
    }

    private void disableInterrupt(int interrupt) {
        int ief = mmu().read(IF)
        mmu().write(ief & ~interrupt, IF)
    }
     ...
     * <b>Flag Alteration:</b>
     * <ul>
     * <li><code>Z</code>: Set if the value before incrementing equals 0xFF</li>
     * <li><code>N</code>: Reset to 0</li>
     * <li><code>H</code>: Set if the value before incrementing equals 0xFF or 0x0F</li>
     * <li><code>C</code>: Unaffected</li>
     * </ul>
     *
     ...
     * <b>Flag Alteration:</b>
     * <ul>
     * <li><code>Z</code>: Set if <code>r</code>'s value before incrementing equals
     * the maximum value of an 8-Bit Register (0xFF)</li>
     * <li><code>N</code>: Reset to 0</li>
     * <li><code>H</code>: Set if <code>r</code>'s value before incrementing equals
     * 0xFF or 0x0F</li>
     * <li><code>C</code>: Unaffected</li>
     * </ul>
     *
     ...
     * <b>Flag Alteration:</b>
     * <ul>
     * <li><code>Z</code>: Unaffected</li>
     * <li><code>N</code>: Unaffected</li>
     * <li><code>H</code>: Unaffected</li>
     * <li><code>C</code>: Unaffected</li>
     * </ul>
     *
     * @param r the register to increment
    ...
    private void add(Register8Bit r, int addValue) {
        ...
        F.set(Z, sum == 0)
        F.set(N, 0)
        F.set(H, ((value & 0x0F) + (addValue & 0x0F)) > 0xF)
        F.set(C, sum > 0xFF)
    }
    ...
    private void add(Register8Bit r1, Register8Bit r2, int addValue) {
        ...
        F.set(N, 0)
        F.set(H, (value & 0x0FFF) + (addValue & 0x0FFF) > 0x0FFF)
        F.set(C, sum > 0xFFFF)
    }

    private void adc(Register8Bit r, int value) {
        int carryBit = F.get(C)
        int value1 = r.read()
        ...
        F.set(Z, sum == 0)
        F.set(N, 0)
        F.set(H, ((value1 & 0x0F) + (value2 & 0x0F)) > 0xF)
        F.set(C, sum > 0xFF)
    }
    ...
    private void sub(Register8Bit r, int subValue) {
        ...
        F.set(Z, difference == 0x00)
        F.set(N, 1)
        F.set(H, (difference & 0xF) > (value & 0xF))
        F.set(C, difference < 0)
    }
    ...
    private void sbc(Register8Bit r, int value) {
        ...
        F.set(Z, difference == 0x00)
        F.set(N, 1)
        F.set(H, (difference & 0xF) > (value1 & 0xF))
        F.set(C, difference < 0)
    }
    ...
    private UInt16 combine(UInt8 hi, UInt8 lo) {
        int _hi = hi.intValue() << 8
        int _lo = lo.intValue() & 0x0FF
