 ...
 * to run javaboy.JavaBoy as an application, and also the necessary applet methods.
 * It also implements a full command based debugger using the console.
 */


public class JavaBoy extends Applet implements Runnable, KeyListener, WindowListener, MouseListener, ActionListener, ItemListener {
     ...
     * A scheme can be activated using the view menu when javaboy.JavaBoy is
                ...
                if (dmgcpu.graphicsChip.isFrameReady()) {
                    ...
                    if (stripTimer < stripLength) {
                        if (stripTimer < stripLength / 2) {
                            bufferGraphics.setColor(new Color(255, 255, 255));
                            bufferGraphics.drawString("javaboy.JavaBoy - Neil Millstone", 2, stripPos + 12);
                            bufferGraphics.setColor(new Color(255, 255, 255));
...
//   g.drawString("www.millstone.demon.co.uk", 2, 126);
            ...
            g.drawString("javaboy.JavaBoy (tm)", 10, 10);
...
//	dmgcpu.execute(-1);
        ...
        } else if (e.getActionCommand().equals("javaboy.JavaBoy Website")) {
            try {
...
//   }
    ...
    public void displayDebuggerHelp() {
        ...
        System.out.println("<CTRL> + C            Quit javaboy.JavaBoy");
    }
...
//           System.out.print(javaboy.JavaBoy.hexByte(unsign(data)));
//           if (st.hasMoreTokens()) System.out.print(", ");
    ...
    /**
     * Initialize javaboy.JavaBoy when run as an application
     */
    public JavaBoy(String cartName) {
        mainWindow = new GameBoyScreen("javaboy.JavaBoy " + versionString, this);
        mainWindow.setVisible(true);
        this.requestFocus();
//  mainWindow.addKeyListener(this);
        mainWindow.addWindowListener(this);
//  cartridge = new javaboy.Cartridge(cartName, mainWindow);
//  dmgcpu = new javaboy.Dmgcpu(cartridge, mainWindow);
    }

    public static void main(String[] args) {
        System.out.println("javaboy.JavaBoy (tm) Version " + versionString + " (c) 2005 Neil Millstone (application)");
...
//  javaBoy.mainWindow.setGraphicsChip(javaBoy.dmgcpu.graphicsChip);
    ...
    public void start() {
        ...
        setupKeyboard();
        System.out.println("javaboy.JavaBoy (tm) Version " + versionString + " (c) 2005 Neil Millstone (applet)");
        ...
        popupMenu.add("javaboy.JavaBoy " + versionString);
        popupMenu.add("-");
        popupMenu.add("Define Controls");
        popupMenu.add(soundCheck = new CheckboxMenuItem("Sound"));
        popupMenu.add("-");
        popupMenu.add("Reset");
        ...
        popupMenu.add("-");
        popupMenu.add("javaboy.JavaBoy Website");
