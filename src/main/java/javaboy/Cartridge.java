...
class Cartridge {
    ...
    /**
     * javaboy.Cartridge type - index into cartTypeTable[][]
     */
    int cartType;
     ...
     * filename given.  Loads via the web if javaboy.JavaBoy is running as an applet
     */
    public Cartridge(String romFileName, Component a) {
        ...
        InputStream is = null;
        try {
/*   if (javaboy.JavaBoy.runningAsApplet) {
    ...
    public InputStream openRom(String romFileName, Component a) {
        ...
        if (bFormat == bNotCompressed) {
            try {
                ...
                if (JavaBoy.runningAsApplet) {
                    return new URL(((Applet) (a)).getDocumentBase(), romFileName).openStream();
                } else {
        ...
        if (bFormat == bZip) {
            ...
            try {
                ...
                if (JavaBoy.runningAsApplet) {
                    zip = new java.util.zip.ZipInputStream(new URL(((Applet) (a)).getDocumentBase(), romFileName).openStream());
                } else {
                    zip = new java.util.zip.ZipInputStream(new FileInputStream(romFileName));
                }
        ...
        if (bFormat == bGZip) {
            ...
            try {
                if (JavaBoy.runningAsApplet) {
                    return new java.util.zip.GZIPInputStream(new URL(((Applet) (a)).getDocumentBase(), romFileName).openStream());
                } else {
                    return new java.util.zip.GZIPInputStream(new FileInputStream(romFileName));
                }
            } catch (Exception e) {
        ...
        switch (cartType) {
            ...
            case 3 /* MBC1+RAM+BATTERY */ :
                out = "MBC1: ROM bank " + JavaBoy.hexByte(currentBank) + " mapped to " +
                        " 4000 - 7FFFF.  ";
                if (mbc1LargeRamMode) {
                    out = out + "javaboy.Cartridge is in 16MBit ROM/8KByte RAM Mode.";
                } else {
                    out = out + "javaboy.Cartridge is in 4MBit ROM/32KByte RAM Mode.";
                }
                return out;
            case 5:
            case 6:
...
//  javaboy.JavaBoy.debugLog(rom[0x14F]+ " "+ rom[0x14E]);
