package javaboy;// Mahjong Quest - Value set at 045E
...
/*  if ((addr >= 0xDFD8) && (addr <= 0xDFF0) && (running)) {
   System.out.println(javaboy.JavaBoy.hexWord(addr) + " read at " + javaboy.JavaBoy.hexWord(pc) + " bank " + cartridge.currentBank);
  }*/

/*  if ((addr < 0) || (addr > 65535)) {
    System.out.println("Tried to read address " + addr + ".  pc = " + javaboy.JavaBoy.hexWord(pc));
    return 0xFF;
  }*/
...
/*  if ((addr >= 0xCFF8) && (addr <= 0xCFF8) && (running)) {
   System.out.println(javaboy.JavaBoy.hexWord(data) + " written to " + javaboy.JavaBoy.hexWord(addr) + " at " + javaboy.JavaBoy.hexWord(pc) + " bank " + cartridge.currentBank);
  }
...
/*  if ((addr < 0) || (addr > 65535)) {
   System.out.println(javaboy.JavaBoy.hexWord(data) + " written to " + javaboy.JavaBoy.hexWord(addr) + " at " + javaboy.JavaBoy.hexWord(pc) + " bank " + cartridge.currentBank);
    ...
    public void checkEnableGbc() {
        if (((cartridge.rom[0x143] & 0x80) == 0x80) && (allowGbcFeatures)) { // GBC javaboy.Cartridge ID
            gbcFeatures = true;
        } else {
            gbcFeatures = false;
        }
...
//	System.out.println("Reg 44 = " + javaboy.JavaBoy.unsign(ioHandler.registers[0x44]));
...
/*   javaboy.GameBoyScreen j = (javaboy.GameBoyScreen) applet;
   if (j.viewFrameCounter.getState()) {
    System.out.print(" " + javaboy.JavaBoy.hexWord(pc) + ":" + javaboy.JavaBoy.hexByte(cartridge.currentBank));
   }*/
//   System.out.print(" " + javaboy.JavaBoy.hexWord(pc) + ":" + javaboy.JavaBoy.hexByte(cartridge.currentBank));
            instrCount++;
...
//        System.out.println("Daa at " + javaboy.JavaBoy.hexWord(pc));
...
//		System.out.println("Halted, pc = " + javaboy.JavaBoy.hexWord(pc));
                    while (ioHandler.registers[0x0F] == 0) {
                        initiateInterrupts();
                        instrCount++;
                    }

//		System.out.println("intrcount: " + instrCount + " IE: " + javaboy.JavaBoy.hexByte(ioHandler.registers[0xFF]));
...
//           System.out.println("SWAP - answer is " + javaboy.JavaBoy.hexByte(data));
                                registerWrite(regNum, data);
                                break;
                            case 0x38:          // SRL r
                                if ((data & 0x01) == 0x01) {
